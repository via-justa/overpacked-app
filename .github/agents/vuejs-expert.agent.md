...
- **State Management**: `@tanstack/vue-query` for server state, Pinia (auth, UI preferences only)
...
- Use `@tanstack/vue-query` for server state; avoid overusing Pinia for local state
- Use Pinia for global state that truly needs to be shared across the app (auth, UI prefs); avoid it for component-specific state
...
- Designing scalable state management strategies that balance Pinia and `vue-query`
