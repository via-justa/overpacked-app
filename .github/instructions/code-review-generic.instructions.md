...
- [ ] When performing a code review, check pull request names follow pattern: (type) ticket-description
- [ ] When performing a code review, verify CI/CD pipeline configuration is correct
- [ ] When performing a code review, check database migrations are reversible
- [ ] When performing a code review, verify API changes are based on updated 'dev/openapi.yaml' spec
- [ ] When performing a code review, verify database schema changes are reflected in 'dev/database-schema.mermaid'
- [ ] When performing a code review, check for use of semantic design tokens in frontend code
- [ ] When performing a code review, verify dockerfiles follow best practices (multi-stage builds, minimal base images)
- [ ] When performing a code review, verify docker images do not contain secrets or sensitive data
- [ ] When performing a code review, verify docker compose files are updated if new services are added, removed, or changed
- [ ] When performing a code review, verify helm charts are updated if new services are added, removed, or changed
...
- **Frontend Tech Stack**: vue.js, Tailwind CSS, Vite
- **Backend Tech Stack**: Go, Gin, GORM
- **Database**: PostgreSQL
- **Architecture**: Monorepo with separate frontend/backend directories, REST API, layered architecture
- **Build Tool**: Makefile for common tasks, Docker for containerization, Helm for Kubernetes deployment
- **Testing**: Go tests with `testing` package, Jest for frontend, integration tests with Testcontainers
- **Code Style**: Go code follows `gofmt` and `golint` conventions, vue.js code follows style guide, consistent use of semantic design tokens in frontend, components organized by feature and decoupled from styling and page layout
