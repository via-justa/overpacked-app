...
1. Editing an existing `.go` file:
	- **PRESERVE** the existing `package` declaration; do not add another one.
	- If replacing the entire file content, start with the same package name already used by that file.
2. Creating a new `.go` file:
	- **BEFORE writing code**, check package names used by other `.go` files in the same directory.
	...
	- Write **exactly one** `package <name>` line at the top of the file.
3. Using file creation or replacement tools:
	- **ALWAYS verify** the target file does not already have a `package` declaration before adding one.
	- If replacing content, include only ONE `package` declaration in the new content.
...
- Prefer letting the caller control concurrency in libraries.
- If a library must create goroutines, document lifecycle and cleanup behavior.
- Always define how each goroutine exits.
- Use `sync.WaitGroup` or channels to coordinate completion.
- Prevent leaks by ensuring cancellation and cleanup paths are present.

### Channels

- Use channels for goroutine communication.
- Prefer communicating by channels over unsynchronized shared memory.
- Close channels from the sender side, not the receiver side.
- Use buffered channels when capacity is known and intentional.
- Use `select` for non-blocking and multiplexed channel operations.

### Synchronization

1. Use `sync.Mutex` to protect shared mutable state and keep critical sections small.
2. Use `sync.RWMutex` when read contention dominates.
3. Choose primitives by purpose: channels for communication, mutexes for state protection.
4. Use `sync.Once` for one-time initialization.
5. Use the `sync.WaitGroup` pattern that matches the module Go version:
   - If `go >= 1.25` in `go.mod`, use `WaitGroup.Go` ([documentation](https://pkg.go.dev/sync#WaitGroup)):
     ```go
...
- Validate input data
- For JSON APIs, use pointers for optional fields
